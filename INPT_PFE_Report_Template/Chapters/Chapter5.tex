...
\textbf{static analysis} of the low-level driver layers of both SDKs. We compare the
...
Every scenario in this chapter targets two boards hosting comparable Cortex-M33 parts.
Table~\ref{tab:ti-board-comparison} summarises their key characteristics.
...
\paragraph{Security analysis (CWE).}
...
Table~\ref{tab:ti-periph-exclusive} lists the peripheral drivers present in only one SDK.
...
\bottomrule
\multicolumn{4}{l}{\footnotesize \textsuperscript{*}After excluding tooling-artifact Rule~17.3 from both vendors.} \\
\end{tabular}
\end{table}
